...
Large language models (LLMs) can generate strong solutions to programming problems, but different solution attempts may use different algorithms and succeed on different parts of the same problem. Our work studies whether an LLM can improve its final solution by combining multiple candidate solution paths. We evaluate the method on 60 competitive programming problems from SubtaskEval using official online judge scores. For each problem, we generate two independent candidate solutions using a subtask-aware baseline strategy (Base) and a progressive subtask strategy (Prog). A new LLM run then analyzes both candidates and produces a synthesized solution (Synth) without receiving execution results or correctness feedback. Across the 60 problems, Base and Prog achieve average scores of 62.4 and 73.4, while Synth achieves 87.6. The number of fully solved problems increases from 33 for Base and 39 for Prog to 48 for Synth. On an additional controlled set of 20 problems, an independent repeat of Base (Repeat) achieves an average score of 63.8, compared with 79.9 for Synth, suggesting that the improvement is not explained only by giving the model another generation attempt. These results show that using multiple candidate solution paths can help an LLM produce stronger programming solutions.
