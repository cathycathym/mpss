...
int main() {
    ...
    /*
      Determine the deterministic earliest-arrival parent of every flight.
      If a same-day departure exists, its minimum arrival time is optimal.
      Otherwise, take the globally minimum arrival on the next day.
    */
    for (int i = 0; i + 1 < E; ++i) {
    ...
    /*
      potential[u] is the sum of 0/T parent-edge weights from u to its
      root. Parents are always one layer farther east.
    */
    for (int i = E - 2; i >= 0; --i) {
        for (int u : layers[i].ids) {
    ...
    /*
      Iterative Euler tour of the reverse forest.
    */
    int timer = 0;
    vector<pair<int, bool>> stack;
    stack.reserve(2 * flights.size());
    ...
    for (const Layer& layer : layers)
        maxLayerSize =
            max(maxLayerSize, int(layer.ids.size()));
    ...
    /*
      Sort each layer by Euler position and build RMQ over
      potential[u] - departure[u].
    */
    ...
    auto rangeMinimum = [&](const Layer& layer, int left, int right) {
        int len = right - left;
        int k = floorLog[len];
        int block = 1 << k;
    ...
    auto solvePair = [&](int startLayer, int finalLayer) -> int64 {
        ...
        if (costMerge <= costFromStart &&
            ...
            /*
              Both lists are in Euler order. Each start node belongs to
              exactly one subtree rooted at the final layer.
            */
            int p = 0;
            ...
            /*
              For every first flight, locate its unique ancestor in the
              final layer by predecessor search on Euler entry times.
            */
            for (int u : start.ids) {
                int pos = int(upper_bound(
                    finish.tins.begin(), finish.tins.end(),
                    flights[u].tin
                ) - finish.tins.begin()) - 1;
            ...
            /*
              For every possible final flight, all valid first flights
              form one Euler interval. Query the minimum of
              potential[u] - departure[u] in that interval.
            */
            for (int v : finish.ids) {
                int left = int(lower_bound(
                    start.tins.begin(), start.tins.end(),
                    flights[v].tin
                ) - start.tins.begin());

                int right = int(upper_bound(
                    start.tins.begin(), start.tins.end(),
                    flights[v].tout
                ) - start.tins.begin());
