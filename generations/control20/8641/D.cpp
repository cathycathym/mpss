...
int main() {
    ...
    auto cheapestAvailable = [&](long long height) -> long long {
        return prefixMinC[available(height) - 1];
    };
    ...
    for (const auto &[rootHeight, rootCost] : bestRootCost) {
        ...
        for (int groupSize = 1; groupSize <= backlog; ++groupSize) {
            ...
            if (scheduled == N) {
                ...
                relax(scheduled, nextHeight, groupSize, cost);
            }
        }
    }
    ...
    for (int scheduled = 2; scheduled < N; ++scheduled) {
        ...
        for (const auto &[key, cost] : dp[scheduled]) {
            states.push_back({
                int(key & MASK),
                long long(key >> SHIFT),
                cost
        ...
        for (const State &state : useful) {
            ...
            if (backlog <= state.width) {
                ...
                if (nextScheduled == N) {
                    ...
                    relax(nextScheduled, nextHeight, state.width, nextCost);
                }
            } else {
