...
class ActiveCells {
    ...
    bool hasBit(int node, int col) const {
        return (bits[0][(size_t)node * width[0] + col / 64] >> (col % 64)) & 1ULL;
    }
    ...
    void clearBit(int node, int col) {
        ...
        for (int level = 0; level < (int)bits.size(); ++level) {
            int word = pos / 64;
            int offset = pos % 64;
            uint64_t &value = bits[level][(size_t)node * width[level] + word];
    ...
    int findNext(int node, int level, int left, int right) const {
        ...
        const uint64_t *data =
            bits[level].data() + (size_t)node * width[level];

        if (firstWord == lastWord) {
            uint64_t lowMask = ~0ULL << firstOffset;
            uint64_t highMask =
                (lastOffset == 63 ? ~0ULL : ((1ULL << (lastOffset + 1)) - 1));
            uint64_t value = data[firstWord] & lowMask & highMask;
            if (value == 0) return -1;
            return firstWord * 64 + __builtin_ctzll(value);
        }

        uint64_t firstValue = data[firstWord] & (~0ULL << firstOffset);
        if (firstValue != 0)
...
public:
    ActiveCells(int r, int c) : rows(r), cols(c) {
        ...
        for (int level = 0; level < (int)width.size(); ++level)
            bits[level].assign((size_t)nodes * width[level], 0);

        uint64_t tailMask =
            (cols % 64 == 0 ? ~0ULL : ((1ULL << (cols % 64)) - 1));

        for (int r0 = 0; r0 < rows; ++r0) {
            int node = base + r0;
            uint64_t *data = bits[0].data() + (size_t)node * width[0];
            fill(data, data + width[0], ~0ULL);
            data[width[0] - 1] &= tailMask;
        }

        for (int node = base - 1; node >= 1; --node) {
            for (int w0 = 0; w0 < width[0]; ++w0) {
                bits[0][(size_t)node * width[0] + w0] =
                    bits[0][(size_t)(node * 2) * width[0] + w0] |
                    bits[0][(size_t)(node * 2 + 1) * width[0] + w0];
            }
        }

        for (int level = 1; level < (int)width.size(); ++level) {
            int previousWidth = width[level - 1];
            int currentWidth = width[level];

            for (int node = 1; node < nodes; ++node) {
                const uint64_t *previous =
                    bits[level - 1].data() + (size_t)node * previousWidth;
                uint64_t *current =
                    bits[level].data() + (size_t)node * currentWidth;
    ...
    void erase(int row, int col) {
        ...
        while (node >= 1) {
            if (hasBit(node * 2, col) || hasBit(node * 2 + 1, col))
                break;
            clearBit(node, col);
            node >>= 1;
        }
    ...
    int findOne(int rowLeft, int rowRight, int colLeft, int colRight) const {
...
int main() {
    ...
    auto discoverRectangle = [&](int r1, int r2, int c1, int c2, int value) {
        r1 = max(r1, 0);
        r2 = min(r2, R - 1);
        c1 = max(c1, 0);
        c2 = min(c2, C - 1);
        ...
        while (true) {
            int id = active.findOne(r1, r2, c1, c2);
            if (id == -1) break;

            int row = id / C;
            int col = id % C;
            active.erase(row, col);
            distance[id] = value;
    ...
    while (!queue.empty()) {
        auto [id, queuedDistance] = queue.front();
        queue.pop_front();

        if (distance[id] != queuedDistance) continue;
        if (id == goal) {
            cout << queuedDistance << '\n';
            return 0;
        }
        ...
        for (int direction = 0; direction < 4; ++direction) {
            ...
            int next = nr * C + nc;
            if (distance[next] > queuedDistance) {
                if (distance[next] == INF)
                    active.erase(nr, nc);
                distance[next] = queuedDistance;
                queue.push_front({next, queuedDistance});
            }
        }

        int nextDistance = queuedDistance + 1;
