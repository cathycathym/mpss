...
public:
    explicit RangeAssignSumTree(int size)
        : n(size),
          sum(4 * size + 5, 0),
          lazy(4 * size + 5, 0),
          hasLazy(4 * size + 5, 0) {}
...
class RangeMinimumTree {
    int size;
    vector<int64> tree;
    static constexpr int64 INF =
        numeric_limits<int64>::max() / 4;
...
int main() {
    ...
    for (int id = 0; id < Q; ++id) {
        int left, right;
        cin >> left >> right;
        queriesByRight[right].push_back({left, id});
    }

    vector<int64> answer(Q, -1);

    RangeMinimumTree rangeMinimum(h);
    RangeAssignSumTree suffixMinimumSums(N);

    // Increasing values of the current suffix-minimum groups.
    // Each pair stores {value, first index of the group}.
    ...
    for (int right = 1; right <= N; ++right) {
        ...
        suffixMinimumSums.assignRange(firstChanged, right, h[right]);
        ...
        for (const Query& query : queriesByRight[right]) {
            ...
            int64 minimumH = rangeMinimum.query(left, right);
            ...
            int64 sumSuffixMinimum =
                suffixMinimumSums.querySum(left, right);

            answer[query.id] = sumH - sumSuffixMinimum;
        }
    }
